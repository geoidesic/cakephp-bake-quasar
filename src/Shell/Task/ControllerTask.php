 ...
 * @todo: for some reason extending the default ControllerTask intead of BakeTask will cause the QuasarAdmin.BakeHelper not to load and instead the default helper is loaded. Haven't been able to figure out why, thus we aren't DRY here
...
class ControllerTask extends BakeTask {
	...
	public function bake($controllerName) {
		...
		$crudPluginLoaded = \Cake\Core\Plugin::isLoaded('Crud');
		...
		$out = $this->bakeControllerApi($controllerName, $data);
	 ...
	 * @return string The generated controller file.
	 */
	public function bakeControllerApi($controllerName, array $data) {
		$data += [
			'name' => null,
			'namespace' => null,
			'prefix' => null,
		...
		$contents = $this->BakeTemplate->generate('Controller/api');

		$path = $this->getPath();
		$filename = $path . $controllerName . 'Controller.php';
		$filename = str_replace('Baked', 'Api', $filename);
