 ...
 */
class QuasarSelectWidget extends BasicWidget
{
    ...
    public function render(array $data, ContextInterface $context)
    {
        ...
        $name = $data['name'];
        $formelementname = $data['name'];
        if (strstr($formelementname, '[')) {
            $name = substr($formelementname, 0, strpos($formelementname, '['));
        }
        $value = $data['val'];
        if (!empty($value) && is_array($value)) {
            ...
            // $value = '['.implode(',', array_map($func, $value)).']';
        }

        // var_dump($value);
        unset($data['empty'], $data['val'], $data['escape'], $data['options'], $data['name']);
        ...
        return $this->_templates->format($template, $params);
    }
    ...
    protected function _renderContent($data)
    {
        ...
        # fetch related name or title instead of id for text
    ...
    protected function _renderOptions($options, $disabled, $selected, $templateVars, $escape)
    {
        ...
        foreach ($options as $key => $val) {
            ...
            $option = $this->_templates->format('option', [
                'value' => $value,
                'text' => $text,
